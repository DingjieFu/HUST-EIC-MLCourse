...
def clusterClubs(numClust=5):
    datList = []
    # 导入数据
    text_path = "PythonDeveloper/Machine_Learning/6_Clustering/task1/places.txt"
    ...
    # 导入地图
    png_path = "PythonDeveloper/Machine_Learning/6_Clustering/task1/Portland.png"
    imgP = plt.imread(png_path)
    ax0.imshow(imgP)
...
def kmeans(numClust=5):
    text_path = "PythonDeveloper/Machine_Learning/6_Clustering/task1/places.txt"
    X = loadDataSet(text_path)
    centroids, clustAssing = kMeans(X, numClust)
    # 不同图形标识
    scatterMarkers = ['s', 'o', '^', '8', 'p',
                      'd', 'v', 'h', '>', '<']
    fig = plt.figure()
    ...
    ax1 = fig.add_axes(rect, label='ax1', frameon=False)
    for i in range(5):
        ptsInCurrCluster = X[nonzero(clustAssing[:, 0].A == i)[0], :]
        markerStyle = scatterMarkers[i % len(scatterMarkers)]
        ax1.scatter(ptsInCurrCluster[:, 0].flatten(
        ).A[0], ptsInCurrCluster[:, 1].flatten().A[0], marker=markerStyle, s=90)
        # 采用‘+’表示簇中心
