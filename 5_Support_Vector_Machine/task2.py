...
def gaussModel(dataPath):
    ...
    for i in range(len(C_list)):
        for j in range(len(sigma_list)):
            model = svmTrain_SMO(X, y, C=C_list[i], kernelFunction='gaussian',
                                 K_matrix=gaussianKernel(X, sigma=sigma_list[j]))  # 模型训练
            pre = svmPredict(model, X, sigma_list[j])
            print(
                f"C={C_list[i]}, sigma={sigma_list[j]}, accuracy={accuracy(pre, y)}")
...
if __name__ == '__main__':
    path = 'PythonDeveloper/Machine_Learning/5_Support_Vector_Machine/task2/task2.mat'
    gaussModel(path)
