...
def loadData(filename):
    """
    输入:数据集路径
    输出:numpy.array格式的X, y数据array
        X为(m,n)的数据array, m为样例数, n为特征维度
        y为(m,1)的标签array, 1表示正例, 0表示反例
    """
...
# 线性SVM
def linearModel(dataPath):
    X, y = loadData(dataPath)  # 载入数据
    # plotData(X, y)  # 观察数据
    model = svmTrain_SMO(X, y, C=100, max_iter=20)  # 模型训练
    visualizeBoundaryLinear(X, y, model, title="C=100")  # 划分超平面


# 高斯核SVM
def gaussModel(dataPath):
    X, y = loadData(dataPath)  # 载入数据
    # plotData(X, y)  # 观察数据
    model = svmTrain_SMO(X, y, C=5, kernelFunction='gaussian',
                         K_matrix=gaussianKernel(X, sigma=0.1))  # 模型训练
    visualizeBoundaryGaussian(X, y, model, sigma=0.1)  # 划分超平面


if __name__ == '__main__':
    linear_path = 'PythonDeveloper/Machine_Learning/5_Support_Vector_Machine/task1/task1_linear.mat'
    gauss_path = 'PythonDeveloper/Machine_Learning/5_Support_Vector_Machine/task1/task1_gaussian.mat'
    # linearModel(linear_path)
    gaussModel(gauss_path)
